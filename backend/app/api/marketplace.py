...
async def get_opportunities(
    ...
    for loan, company, listing in results:
        ...
        opportunities.append(LoanOpportunity(
            loan_id=loan.id,
            company_id=company.sme_id,
            sector=company.sector,
            region=company.region,
            seller_lender=anonymize_lender(
                db.query(Lender).filter(Lender.id == loan.current_lender_id).first().name
            ),
...
async def get_my_loans(
    lender_id: int = Query(..., description="Current lender ID"),
    mismatched_only: bool = Query(True, description="Only show mismatched loans"),
    db: Session = Depends(get_db)
):
    """Get current lender's loans, optionally filtered to mismatches"""
    query = (
    ...
    if mismatched_only:
        query = query.filter(Loan.is_mismatch == True)
    ...
    for loan, company in results:
        # Check if listed
        listing = db.query(ListedLoan).filter(
            ListedLoan.loan_id == loan.id,
            ListedLoan.is_active == True
        ).first()
        ...
        if loan.best_match_lender_id:
            best_match = db.query(Lender).filter(Lender.id == loan.best_match_lender_id).first()
            best_match_name = anonymize_lender(best_match.name) if best_match else None

        my_loans.append(MyLoan(
            loan_id=loan.id,
            company_id=company.sme_id,
            sector=company.sector,
            region=company.region,
            ...
            bid_count=len(bids),
            best_bid_discount=min([b.discount_percent for b in bids]) if bids else None,
        ))
...
async def list_loan(
    ...
    if loan.current_lender_id != request.lender_id:
        raise HTTPException(status_code=403, detail="Loan does not belong to this lender")

    # Check if already listed
    existing = db.query(ListedLoan).filter(
        ListedLoan.loan_id == request.loan_id,
        ListedLoan.is_active == True
    ).first()
    if existing:
...
async def reveal_identity(
    ...
    return {
        ...
        "seller_name": db.query(Lender).filter(Lender.id == reveal.seller_lender_id).first().name if reveal.both_revealed else None,
        "buyer_name": db.query(Lender).filter(Lender.id == reveal.buyer_lender_id).first().name if reveal.both_revealed else None,
    }
