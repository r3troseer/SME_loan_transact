...
def generate_loan_explanation_template(loan, company, current_lender, best_lender) -> str:
    """Generate explanation using template (fallback)"""
    reasons = []

    if loan.fit_gap and loan.fit_gap > 20:
        reasons.append(f"This loan shows a significant fit improvement of {loan.fit_gap:.0f}% with the recommended lender.")

    if company.inclusion_score and company.inclusion_score >= 60:
        reasons.append(f"The company has a high inclusion priority score of {company.inclusion_score:.0f}, indicating it serves an underserved market.")

    if company.risk_score and company.risk_score >= 60:
        reasons.append(f"With a risk score of {company.risk_score:.0f}, this company demonstrates solid financial health.")

    if best_lender and best_lender.inclusion_mandate:
        reasons.append(f"The recommended lender has an inclusion mandate, making this a mission-aligned opportunity.")

    if not reasons:
        reasons.append("This reallocation would optimize portfolio fit for both parties.")
...
Explain why this reallocation benefits both parties and the SME. Focus on risk alignment, sector expertise, and inclusion impact."""
    ...
    explanation = generate_loan_explanation_template(loan, company, current_lender, best_lender)
    return ExplanationResponse(
        loan_id=request.loan_id,
        explanation=explanation,
...
async def generate_market_insight(
    ...
    avg_fit_gap = db.query(func.avg(Loan.fit_gap)).filter(Loan.is_mismatch == True).scalar() or 0
...
Mismatched Loans: {mismatched_loans} ({mismatched_loans/total_companies*100:.1f}%)
...
Be specific about opportunities and actionable insights."""
    ...
    # Fallback
    insight = f"The market shows {mismatched_loans} loans with reallocation potential, representing a {avg_fit_gap:.1f}% average fit improvement opportunity. {high_inclusion} companies qualify as high inclusion priority."
    return MarketInsightResponse(
        insight=insight,
        generated_by="template"
    )
...
async def generate_swap_story(
    request: SwapStoryRequest,
    db: Session = Depends(get_db)
):
...
Sector: {company.sector}
Region: {company.region}
Risk Score: {company.risk_score:.0f}/100
Inclusion Score: {company.inclusion_score:.0f}/100
Inclusion Flags: {', '.join(company.inclusion_flags or ['None'])}
Current Lender Fit: {loan.current_lender_fit:.0f}% (Fit Gap: {loan.fit_gap:.0f}%)
...
Provide actionable insight about this company's financial health and lending alignment."""
    ...
    # Fallback
    insight = f"This {company.sector} company in {company.region} shows {'strong' if company.risk_score >= 60 else 'moderate'} financial health with a {loan.fit_gap:.0f}% potential fit improvement through reallocation."
    return CompanyInsightResponse(
        company_id=request.company_id,
        insight=insight,
        generated_by="template"
