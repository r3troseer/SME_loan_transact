...
def simulate_loans(df: pd.DataFrame, lender_names: list) -> pd.DataFrame:
    ...
    df['Outstanding_Balance'] = df.apply(
        lambda row: row['Loan_Amount'] * (row['Years_Remaining'] / row['Loan_Term_Years'])
        if row['Loan_Term_Years'] > 0 else 0,
        axis=1
    )

    # Monthly payment (simplified)
    df['Monthly_Payment'] = df.apply(
        lambda row: (row['Loan_Amount'] * (1 + row['Interest_Rate'] * row['Loan_Term_Years']))
        / (row['Loan_Term_Years'] * 12) if row['Loan_Term_Years'] > 0 else 0,
        axis=1
    )
...
def run_analysis_pipeline(df: pd.DataFrame, lenders: list) -> pd.DataFrame:
    ...
    if not AGENTS_AVAILABLE:
        ...
        df['Risk_Category'] = df['Risk_Score'].apply(
            lambda x: 'Low Risk' if x >= 60 else ('Moderate Risk' if x >= 40 else 'High Risk')
        )
    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            company = Company(
                ...
                inclusion_flags=row.get('Inclusion_Flags') if isinstance(row.get('Inclusion_Flags'), list) else None,
            )
            db.add(company)
            db.flush()
            ...
            loan = Loan(
                ...
                current_fit_reasons=row.get('Current_Fit_Reasons') if isinstance(row.get('Current_Fit_Reasons'), dict) else None,
                best_match_lender_id=best_match_lender_id,
                best_match_fit=row.get('Best_Match_Fit'),
                best_match_reasons=row.get('Best_Match_Reasons') if isinstance(row.get('Best_Match_Reasons'), dict) else None,
...
def run_migration(excel_path: str = None):
    ...
    if AGENTS_AVAILABLE:
        ...
        lenders_data = [
            {"name": "Alpha Bank", "description": "Conservative lender", "risk_tolerance": "low", "risk_score_min": 60},
            {"name": "Growth Capital Partners", "description": "Growth-focused", "risk_tolerance": "high", "risk_score_min": 30},
            {"name": "Regional Development Fund", "description": "Regional focus", "risk_tolerance": "medium", "inclusion_mandate": True},
            {"name": "Sector Specialist Credit", "description": "Sector specialist", "risk_tolerance": "medium"},
        ]
