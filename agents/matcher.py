"""
Agent 3: Matcher
Calculates fit scores between companies and lenders, identifies mismatches,
...
from utils.anonymizer import (
    anonymize_lender, round_score, band_turnover, group_region,
    anonymize_fit_reason, reset_lender_mapping
)
...
class Matcher:
    ...
    Identifies mismatches and recommends reallocations.
    """
    ...
    def analyze(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['Current_Lender_Fit'], df['Current_Fit_Reasons'] = zip(*df.apply(
        ...
        # Is it a mismatch?
        df['Is_Mismatch'] = df['Fit_Gap'] > self.fit_threshold_moderate

        return df

    def _calculate_fit(self, company: pd.Series, lender: dict) -> Tuple[float, List[str]]:
        """
        ...
        if risk_score >= risk_min:
            ...
            if gap <= 10:
                risk_points = 20
                mismatch_reasons.append(f"Risk score {risk_score:.0f} slightly below {risk_min}")
            elif gap <= 20:
                risk_points = 10
                mismatch_reasons.append(f"Risk score {risk_score:.0f} below threshold {risk_min}")
            else:
                risk_points = 0
                mismatch_reasons.append(f"Risk score {risk_score:.0f} significantly below {risk_min}")
        ...
        if preferred_sectors is None:
            ...
            mismatch_reasons.append(f"Sector '{company_sector}' not in lender's focus: {preferred_sectors}")
        ...
        if preferred_regions is None:
            ...
            mismatch_reasons.append(f"Region '{company_region}' outside lender's focus")
        ...
        if size_match:
            ...
            if turnover < min_turnover:
                mismatch_reasons.append(f"Company too small (£{turnover/1e6:.1f}m < £{min_turnover/1e6:.1f}m min)")
            elif max_turnover and turnover > max_turnover:
                mismatch_reasons.append(f"Company too large (£{turnover/1e6:.1f}m > £{max_turnover/1e6:.1f}m max)")
    ...
    def get_reallocation_recommendation(self, company: pd.Series, anonymize: bool = False) -> Dict:
        """
        ...
        return {
            ...
            'status': status,
            'is_mismatch': company.get('Is_Mismatch', False),
            'all_lender_fits': anon_all_fits
        }

    def get_reallocation_candidates(self, df: pd.DataFrame, status_filter: str = None) -> pd.DataFrame:
        """
        ...
        """
        candidates = df[df['Is_Mismatch'] == True].copy()

        if status_filter == 'STRONG':
            candidates = candidates[candidates['Reallocation_Status'] == 'STRONG REALLOCATION CANDIDATE']
        elif status_filter == 'MODERATE':
            candidates = candidates[candidates['Reallocation_Status'].str.contains('CANDIDATE')]
    ...
    def get_market_summary(self, df: pd.DataFrame, anonymize: bool = False) -> Dict:
        ...
        """
        total = len(df)
        mismatches = len(df[df['Is_Mismatch'] == True])
        strong_candidates = len(df[df['Reallocation_Status'] == 'STRONG REALLOCATION CANDIDATE'])
        moderate_candidates = len(df[df['Reallocation_Status'] == 'MODERATE REALLOCATION CANDIDATE'])
        ...
        for lender_name in LENDERS.keys():
            ...
            display_name = anonymize_lender(lender_name, is_current=False) if anonymize else lender_name
        ...
        return {
            ...
            'fit_scores': {
                'average_current_fit': round_score(avg_current_fit) if anonymize else round(avg_current_fit, 1),
                'average_optimal_fit': round_score(avg_best_fit) if anonymize else round(avg_best_fit, 1),
                'average_improvement': avg_imp_display
            },
            'lender_flows': lender_stats,
            'reallocation_value': {
                'total_outstanding': round(total_reallocation_value, 2) if not anonymize else None,
...
if __name__ == "__main__":
    ...
    for idx, row in candidates.head(5).iterrows():
        ...
        print(f"  Current: {rec['current_situation']['lender']} (Fit: {rec['current_situation']['fit_score']})")
        print(f"  Recommended: {rec['recommendation']['lender']} (Fit: {rec['recommendation']['fit_score']})")
