...
class Pricer:
    ...
    def analyze(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["Default_Probability"] = df["Risk_Score"].apply(
            self._estimate_default_probability
        )
        ...
        df["Expected_Loss"] = df.apply(
            lambda row: row["Default_Probability"]
        ...
        df["Misfit_Discount"] = df["Current_Lender_Fit"].apply(
            self._calculate_misfit_discount
        )
        ...
        df["Discount_Percent"] = (
            1 - df["Suggested_Price"] / df["Outstanding_Balance"]
        ) * 100
    ...
    def get_pricing_details(
        self, company: pd.Series, anonymize: bool = False, for_table: bool = False
    ) -> Dict:
        """
        ...
        return {
            ...
            "loan_details": {
                "original_amount": company.get("Loan_Amount", 0)
                if not anonymize
                else None,
                "outstanding_balance": outstanding_display,
                "years_remaining": company.get("Years_Remaining", 0),
                "monthly_payment": company.get("Monthly_Payment", 0)
                if not anonymize
                else None,
                "interest_rate": company.get("Interest_Rate", 0)
                if not anonymize
                else None,
            },
            "valuation": {
                "remaining_payments": company.get("Remaining_Payments", 0)
                if not anonymize
                else None,
                "gross_value": company.get("Gross_Loan_Value", 0)
                if not anonymize
                else None,
                "expected_loss": company.get("Expected_Loss", 0)
                if not anonymize
                else None,
                "risk_adjusted_value": company.get("Risk_Adjusted_Value", 0)
                if not anonymize
                else None,
            },
            "pricing": {
                "misfit_discount": company.get("Misfit_Discount", 0)
                if not anonymize
                else None,
            ...
            "buyer_metrics": {
                ...
                "risk_adjusted_roi": company.get("Risk_Adjusted_ROI", 0)
                if not anonymize
                else None,
                "annualized_roi": roi_display,
                "default_probability": company.get("Default_Probability", 0)
                if not anonymize
                else None,
            },
            "risk_context": {
    ...
    def get_transaction_summary(
        ...
        """
        pricing = self.get_pricing_details(
            company, anonymize=anonymize, for_table=False
        )
        ...
        if anonymize:
            ...
            outstanding_display = pricing["loan_details"][
                "outstanding_balance"
            ]  # Already formatted
            ...
            fit_improvement = round_score(
                pricing["risk_context"]["best_fit"]
            ) - round_score(pricing["risk_context"]["current_fit"])
        else:
            display_buyer = best_lender
            outstanding_display = self.format_price(
                company.get("Outstanding_Balance", 0)
            )
            ...
            fit_improvement = company.get("Best_Match_Fit", 0) - company.get(
                "Current_Lender_Fit", 0
            )
        ...
        if transaction_type == "swap":
            summary["note"] = (
                "Swap transaction - look for matching loan from buyer's portfolio"
            )
    ...
    def get_market_pricing_stats(
        self, df: pd.DataFrame, anonymize: bool = False
    ) -> Dict:
        """
        ...
        return {
            ...
            "discount_distribution": {
                ...
                "5-10%": len(
                    candidates[
                        (candidates["Discount_Percent"] >= 5)
                        & (candidates["Discount_Percent"] < 10)
                    ]
                ),
                "10-15%": len(
                    candidates[
                        (candidates["Discount_Percent"] >= 10)
                        & (candidates["Discount_Percent"] < 15)
                    ]
                ),
                "15-20%": len(
                    candidates[
                        (candidates["Discount_Percent"] >= 15)
                        & (candidates["Discount_Percent"] < 20)
                    ]
                ),
            ...
            "roi_distribution": {
                ...
                "5-10%": len(
                    candidates[
                        (candidates["Annualized_ROI"] >= 5)
                        & (candidates["Annualized_ROI"] < 10)
                    ]
                ),
                "10-15%": len(
                    candidates[
                        (candidates["Annualized_ROI"] >= 10)
                        & (candidates["Annualized_ROI"] < 15)
                    ]
                ),
...
if __name__ == "__main__":
    ...
    candidate = df[df["Is_Unalign"] == True].iloc[0]
    ...
    print(
        f"Outstanding: {pricer.format_price(details['loan_details']['outstanding_balance'])}"
    )
    print(
        f"Suggested Price: {pricer.format_price(details['pricing']['suggested_price'])}"
    )
